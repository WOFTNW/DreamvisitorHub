...
public class DiscEventListener extends ListenerAdapter {
    ...
    public void onMessageReceived(@Nonnull MessageReceivedEvent event) {
        ...
        try {
            ...
            System.out.println("Whitelist ID: " + Bot.getWhitelistChannel().getId());
            System.out.println("Game chat ID: " + Bot.getGameChatChannel().getId());
            System.out.println("Log chat ID: " + Bot.getGameLogChannel().getId());
            if (channel.getId().equals(Bot.getWhitelistChannel().getId()) && !user.isBot() && !p.matcher(username).find()) {
                ...
                System.out.println("Checking for valid UUID");
                ...
                if (uuid == null) {
                    ...
                    event.getMessage().addReaction(Emoji.fromFormatted("❌")).queue();
                    System.out.println("Failed whitelist.");
                } else {

                    System.out.println("Got UUID");

                    // Link accounts if not already linked
                    System.out.println("Do accounts need to be linked?");
                    if (AccountLink.getUuid(user.getIdLong()) == null) {
                        System.out.println("Yes, linking account.");
                    ...
                    try {
                        if (Whitelist.isUserWhitelisted(uuid)) {
                            System.out.println("Already whitelisted.");
                        ...
                        if (StorageManager.isDebug()) throw new RuntimeException();
                    }
                }
            ...
            System.out.println("Dreamvisitor does not have sufficient permissions in the whitelist channel! " + e.getMessage());
        }
        ...
        if (channel.getId().equals(Bot.getGameChatChannel().getId()) && !user.isBot()
                && !StorageManager.isPauseChat()) {

            // TODO: Send message to plugin
...
//            });
        ...
        if (event.getChannel().getId().equals(Bot.getGameLogChannel().getId())) {
            ...
            if (StorageManager.isEnableLogConsoleCommands() && Objects.requireNonNull(event.getMember()).hasPermission(Permission.ADMINISTRATOR)) {

                System.out.println("Sending console command from log channel...");

                String message = event.getMessage().getContentRaw();

                // TODO: Running commands from log channel
...
> *The earth is rumbling...*"""
    ...
    public void onButtonInteraction(@NotNull ButtonInteractionEvent event) {
        ...
        System.out.println("Button interaction with ID " + event.getButton().getId());

        Button button = event.getButton();
        ButtonInteraction interaction = event.getInteraction();

        if (Objects.equals(button.getId(), "panic")) {
            // TODO: Panic kick all players and set player limit
...
//                banList.addBan(Bukkit.getServer().createPlayerProfile(username), "Banned by Dreamvistitor.", (Date) null, null);
            ...
            if (SessionData.autoRestart) {
                SessionData.autoRestart = false;
