...
public class DCmdSetwhitelist implements DiscordCommand {
  ...
  public void onCommand(@NotNull SlashCommandInteractionEvent event) {
    try {
      ...
      // Set the whitelist channel - this will update both PocketBase and the file
      // config
      Bot.setWhitelistChannel(channel);

      // Reply with success message
      event.reply("Whitelist channel set to " + Bot.getWhitelistChannel().getAsMention() +
          ". Configuration has been updated in both PocketBase and the config file.").queue();

      logger.info("Whitelist channel set to " + channel.getName() + " (" + channel.getId() + ")");
      Bot.sendLog("Whitelist channel set to " + channel.getName() + " (" + channel.getId() + ")");
    } catch (Exception e) {
      // Handle any exceptions
      logger.log(Level.SEVERE, "Error setting whitelist channel", e);
      event.reply("An error occurred while setting the whitelist channel: " + e.getMessage()).setEphemeral(true)
          .queue();
    }
  }
}
